...
import { Beaker, Upload, DollarSign, Info, CheckCircle, FileJson, FileCode, CheckCircle2, Loader2, Database, Download, ShieldAlert } from 'lucide-react';
...
import { getLabDatasets, getDatasetPayload } from '../services/apiService';
...
export default function LabDashboard() {
  ...
  const loadDatasets = async () => {
    try {
      ...
      const data = await getLabDatasets(address!);
      setDatasets(data);
    } catch (err) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 animate-in fade-in duration-500">
        ...
        <p className="text-[var(--text-muted)] max-w-md text-center">
          The Lab Dashboard is exclusively available to registered AI Labs. Please connect with an approved AI Lab wallet.
        </p>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex items-center gap-4 mb-2">
        ...
        <div>
          ...
          <p className="text-[var(--text-muted)]">Manage your models and process blind inferences.</p>
        </div>
      </div>

      <div className="flex space-x-2 border-b border-white/10 pb-4 mb-8 relative">
        <button 
          onClick={() => setActiveTab('upload')}
          className={`px-4 py-2 text-sm font-bold uppercase tracking-wider rounded-lg transition-colors relative ${activeTab === 'upload' ? 'bg-[var(--accent-cyan)]/10 text-[var(--accent-cyan)]' : 'text-[var(--text-muted)] hover:bg-white/5'}`}
        >
          Register Model
        </button>
        <button 
          onClick={() => setActiveTab('datasets')}
          className={`px-4 py-2 text-sm font-bold uppercase tracking-wider rounded-lg transition-colors relative ${activeTab === 'datasets' ? 'bg-[var(--accent-cyan)]/10 text-[var(--accent-cyan)]' : 'text-[var(--text-muted)] hover:bg-white/5'}`}
        >
          Received Datasets
        </button>
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'datasets' && (
          <motion.div 
            ...
            className="space-y-6"
          >
            <Card className="bg-[var(--bg-secondary)]/30">
              <div className="flex items-center gap-3 mb-6 pb-4 border-b border-white/5">
                 <Database className="w-5 h-5 text-[var(--accent-cyan)]" />
                 <h2 className="text-xl font-bold tracking-tight">Assigned Datasets</h2>
              </div>
              {isLoadingDatasets ? (
                ...
                <div className="text-center py-12 text-[var(--text-muted)] space-y-2">
                  ...
                  <p>No datasets received yet.</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm text-left">
                    <thead className="text-xs uppercase text-[var(--text-muted)] border-b border-white/10">
                      <tr>
                    ...
                    <tbody>
                      {datasets.map((ds) => (
                        <tr key={ds.dataset_id} className="border-b border-white/5 bg-white/[0.01] hover:bg-white/[0.02]">
                          <td className="px-4 py-3 font-medium text-white">{ds.filename}</td>
                          <td className="px-4 py-3 font-mono text-xs text-[var(--text-muted)]">{ds.columns?.length || 0} cols</td>
                          <td className="px-4 py-3 font-mono text-xs text-[var(--text-muted)]">{ds.owner_address.substring(0, 12)}...</td>
                          <td className="px-4 py-3 text-[var(--text-muted)] text-xs">{new Date(ds.created_at).toLocaleString()}</td>
                          <td className="px-4 py-3">
                            <Button 
                              size="sm" 
                              variant="secondary" 
                              onClick={() => handleDownloadDataset(ds.dataset_id, ds.filename)}
                              isLoading={downloadingId === ds.dataset_id}
